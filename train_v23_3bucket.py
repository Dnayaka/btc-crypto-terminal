...
tb = 2.0
tm_opts = [2.2, 2.4, 2.6]
th_opts = [2.6, 2.8, 3.0, 3.2, 3.4, 3.6]
w1 = 0.40
w2_opts = [0.55, 0.60, 0.65]

print("Starting Train Search 3-Bucket (2019-2023)...")
train_results = []
for tm, th, w2 in itertools.product(tm_opts, th_opts, w2_opts):
    if tm >= th: continue
    cf = dict(base_cf)
    cf['add_long'] = pb_l_tr
    ...
    # 3-bucket array
    tp_arr = np.where(atr_tr > w2, th, np.where(atr_tr > w1, tm, tb))
    cf['tp'] = tp_arr
    
    res, _ = eng.run(df_train, cf)
    if res['dd'] > 0:
        train_results.append({
            'tb': tb, 'tm': tm, 'th': th, 'w1': w1, 'w2': w2,
...
print("\n--- TOP 5 RETURN CONFIGS IN TRAIN (3-BUCKET) ---")
print(r_tr.head(5))

# Baseline v22 (2-bucket: SL 1.9, TB 2.0, TG 2.4, W1 0.40)
cf_base = dict(base_cf)
cf_base['add_long'] = pb_l_tr
cf_base['add_short'] = pb_s_tr
cf_base['tp'] = np.where(atr_tr > 0.40, 2.4, 2.0)
res_base, _ = eng.run(df_train, cf_base)
print("\n--- BASELINE V22 MAX CALMAR IN TRAIN ---")
print(res_base)

if len(r_tr) > 0 and r_tr.iloc[0]['ret'] > res_base['ret'] + 10:
    best = r_tr.iloc[0]
    btm, bth, bw2 = best['tm'], best['th'], best['w2']
    print(f"\nFOUND BETTER 3-BUCKET: TM={btm} TH={bth} W2={bw2}")
    
    # Eval on Test
    cf_t = dict(base_cf)
    cf_t['add_long'] = pb_l_ts
    cf_t['add_short'] = pb_s_ts
    cf_t['tp'] = np.where(atr_ts > bw2, bth, np.where(atr_ts > w1, btm, tb))
    rt, _ = eng.run(df_test, cf_t)
    print("Test Holdout (2024-2026):", rt)
    
    # Eval Full
    cf_f = dict(base_cf)
    cf_f['add_long'] = pb_l_f
    cf_f['add_short'] = pb_s_f
    cf_f['tp'] = np.where(atr_f > bw2, bth, np.where(atr_f > w1, btm, tb))
    rf, _ = eng.run(df, cf_f)
    print("Full Period:", rf)
else:
    print("NO 3-BUCKET BEATS V22 SIGNIFICANTLY.")
