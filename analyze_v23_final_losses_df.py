...
# Run full backtest engine
res, tdf = eng.run(df, cf)

# Filter losses
losses = tdf[tdf['ret'] < 0].copy()
wins = tdf[tdf['ret'] > 0].copy()

# Add metrics to DataFrame
losses['dur'] = losses['exit_bar'] - losses['entry_bar']
wins['dur'] = wins['exit_bar'] - wins['entry_bar']

# Calculate ADX / Choppiness approximation at entry (EMA 200 slope)
c = df['close'].values
...
# Extract Months for seasonality
...
# Count how many losses occurred immediately after another loss
tdf_sorted = tdf.sort_values('entry_bar')
tdf_sorted['prev_ret'] = tdf_sorted['ret'].shift(1)
consecutive = tdf_sorted[(tdf_sorted['ret'] < 0) & (tdf_sorted['prev_ret'] < 0)]
