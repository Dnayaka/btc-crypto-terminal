...
def get_sig(df_period, cf, pb_win, pb_dep):
...
w_opts = [10, 20, 28, 35, 45, 60]
d_opts = [0.5, 0.8, 1.0, 1.2, 1.5, 2.0]

print("Starting PB Train Search...")
res_list = []
for w, d in itertools.product(w_opts, d_opts):
    pb_l, pb_s, atr = get_sig(df_train, base_cf, w, d)
    cf = dict(base_cf)
    cf['add_long'] = pb_l
    cf['add_short'] = pb_s
    ...
    if res['dd'] > 0:
        res_list.append({
            'win': w, 'dep': d, 'ret': res['ret'], 'dd': res['dd'], 'wr': res['wr'], 'calmar': res['calmar']
        })

r_df = pd.DataFrame(res_list)
r_df.sort_values('ret', ascending=False, inplace=True)
print("\n--- TOP PB CONFIGS IN TRAIN ---")
print(r_df.head(5))
